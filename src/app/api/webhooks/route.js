...
export async function POST(req) {
  ...
  if (!SIGNING_SECRET) {
    throw new Error('777Error: Please add SIGNING_SECRET from Clerk Dashboard to .env or .env')
  }
  ...
  if (!svix_id || !svix_timestamp || !svix_signature) {
    return new Response('777Error: Missing Svix headers', {
      status: 400,
    })
  }
  ...
  try {
    ...
    console.error('777Error: Could not verify webhook:', err)
    return new Response('777Error: Verification error', {
      status: 400,
    })
  }
...
if (eventType === 'user.created' || eventType === 'user.updated') {
  ...
  return new Response('777Webhook received', { status: 200 })
}